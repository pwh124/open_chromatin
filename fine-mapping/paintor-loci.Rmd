...
```{r, writing out 0.6 snps}
final.mat <- mod.mat %>%
  dplyr::select(chr=chr.sum,pos=bp.sum,rsid=rsid.proxy,A1=a1,A2=a2,Zscore,id=id.sum,lead.snp,r.squared,logP) %>%
...
final.mat[(duplicated(final.mat[,c(3,8)])),]
final.mat[(duplicated(final.mat[,c(3,8)],fromLast = T)),] # none of the proxy SNPs are duplicated

mat.sum <- final.mat %>%
  dplyr::group_by(lead.snp) %>%
  dplyr::summarise(snps=n())

for (name in levels(as.factor(final.mat$lead.snp))){
  #Subset the data by MP
  tmp=subset(final.mat,lead.snp==name) %>% arrange(chr,pos)
