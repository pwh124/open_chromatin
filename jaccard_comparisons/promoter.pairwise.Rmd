...
x <- read.table("../final.data/prom.matrix")
...
x.names <- x %>%
  ...
  dplyr::mutate(eid=str_replace(eid,"regions_prom_","")) %>%
...
# Making a dataframe so clustering results can be used as annotation
my_cells_col <- dendextend::cutree(tree=as.dendrogram(my_hclust_cells),k=3, order_clusters_as_data = FALSE)
...
pdf(file = "../final.results/promoter-heatmap.pdf",width = 12, height = 20)
pheno.map
dev.off()
```