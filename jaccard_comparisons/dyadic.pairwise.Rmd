...
x <- read.table("../final.data/dyadic.matrix")
...
x.names <- x %>%
  ...
  dplyr::filter(str_detect(eid,"regions")) %>%
  # Remove prefix from names
  dplyr::mutate(eid=str_replace(eid,"regions_dyadic_","")) %>%
...
row.names(x.names) <- x.names$Name
x.names <- x.names[,-1]

# Now annotate the ATAC-seq samples with the names used in the paper
# First transpose the matrix
flip <- tibble::as_tibble(t(x.names),rownames = "filename") %>%
  ...
  # Change colun to rownames and transpose back
...
pdf(file = "../final.results/dyadic-heatmap.pdf",width = 12, height = 20)
pheno.map
dev.off()
```

Making the heatmap smaller so it will be better for a published figure
```{r}
...
pdf(file = "../final.results/publication-dyadic-heatmap.pdf",width = 10, height = 6)
new.map
dev.off()
```