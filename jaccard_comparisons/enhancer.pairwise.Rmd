...
x.names <- x %>%
  ...
  dplyr::mutate(eid=str_replace(eid,"regions_enh_","")) %>%
...
pdf(file = "../final.results/enhancer-heatmap.pdf",width = 12, height = 20)
pheno.map
dev.off()
```

Making a heatmap that will more easily fit into a published figure
```{r}
...
pdf(file = "../final.results/publication-enhancer-heatmap.pdf",width = 10, height = 6)
new.map
dev.off()
```